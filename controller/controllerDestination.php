<?php 
    ...
    }elseif(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=="modifDestination"){
        if (!empty($_POST['lieu'])       && isset($_POST['lieu'])    &&
                !empty($_POST['region'])     && isset($_POST['region'])  &&
                !empty($_POST['image'])     && isset($_POST['image'])  &&
                !empty($_POST['petiteDescription'])     && isset($_POST['petiteDescription'])  &&
                ...
                try {
                    ServiceDestination :: serviceUpdateDestination($idDestination, $region,  $lieu,  $image,  $petiteDescription,  $description, $atout1,  $atout2,  $atout3, $lien,  $extraitForum) ;
                } catch(ServiceException $ce) {
                    echo 'Error';
                }
            }
